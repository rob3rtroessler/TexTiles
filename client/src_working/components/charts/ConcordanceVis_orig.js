...
import Tooltip from "../unused/Tooltip";
...
class ConcordanceVis extends React.Component {
  ...
  renderD3 = mode => {
    ...
    // console.log('in renderD3', this.props)
    ...
    rect = rect
      .enter()
      .append("rect")
      .attr("class", d => `data data-${d.data.x} test`)
      .attr("x", d => x(d.data.x))
      .attr("y", graphHeight)
...
ConcordanceVis.defaultProps = {
  chart: LOADING
};

export default withFauxDOM(
  withD3Renderer({ updateOn: ["data", "xLabel", "yLabel"] })(ConcordanceVis)
);
